...
# 我明白Django有自带与数据库交互的功能，但这个模块自我学习python开始，在我明白轮子为何物之前就已建立，其中的函数我都已经非常熟悉，
# 在网站开发的初期我把大部分时间都花在了调整UI、爬虫的功能细节效果上，所以暂时使用了熟悉的这个模块，而不是Django自带的数据库交互模型来完成读取数据的工作。
# 不过我估计很快就能有时间用自带的数据库模型更换掉这个函数
# I understand Django had an build-in database interact module, but this module had been built since I start learning python, I'm already fimilar with it's function.
# at the very first time of development of this site, I spend most of my time on adjust UI/the crawler's working detail, so I used this module to doing the database job.
# I assume I will replace it by the build-in module anytime soon.
...
class Connpsql(object):
    def __init__(self, database='databasename', host='localhost', maintainconnect=False):
    ...
    def readfromtable(self, table, column="*", columnofselect="'*'", contentofselect="*", mode='blurry'):
        ...
        try:
            if mode == 'blurry':
                word = "SELECT {0} FROM {1} WHERE {2} LIKE '%{3}%' ORDER BY release_time_detected DESC;".format(column, table, columnofselect, contentofselect)
            else:
                word = "SELECT {0} FROM {1} WHERE {2} = '{3}' ORDER BY release_time_detected DESC;".format(column, table, columnofselect, contentofselect)
    ...
    def searchtable(self, table, column='*', rownametuple=(), searchword='', limit=0, offset=0):
        word = 'SELECT {0} FROM {1}'.format(column, table)
        word += ' ORDER BY release_time_detected DESC'
        if searchword:
