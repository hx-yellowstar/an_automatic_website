...
class UrlAcquire(object):
    ...
    def requireurl(self):
        ...
        if self.browserusing is None:
            try:
                responsesourcecode = requests.get(targeturl, headers=HEADERS)
            except Exception as e:
        ...
        # if responsesourcecode.text is None:
        #     responsesourcecode = network.RequestMiddleware().setrequest(urlnextstatus[0], browser='PhantomJS')
        #     if responsesourcecode.url != urlnextstatus[0]:
        #         return
        urlnextstatus = URLNextStatus(targeturl=targeturl, depth=depth, sourcecode=responsesourcecode, urltoseenum=urltoseenum)
        # 本来urlnextstatus是一个两个元素的tuple，在这里加上了第三个元素：页面源代码和第四个元素：剩余需遍历网址的数量
